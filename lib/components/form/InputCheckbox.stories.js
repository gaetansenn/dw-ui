...
const TemplateValidation = (args, { argTypes }) => ({
  props: Object.keys(argTypes),
  template: '<div><DwInputCheckbox v-model="value" ref="input" name="input-checkbox" label="Input checkbox" :size="size" :required="required" required /><br><DwButton @click.native="$refs.input.validate()" class="mt-2">Submit</DwButton></div>'
})
