...
export default {
  data () {
    const path = this.$vnode.componentOptions.Ctor.options.configPath || this.$vnode.componentOptions.propsData.configPath

    return {
      path,
      config: loadStaticConfig(this.$dewib.ui.config[path], {})
    }
  },
  created () {
    loadDynamicConfig.call(this, this.$dewib.ui.config[this.path], {}, '')
  }
}
