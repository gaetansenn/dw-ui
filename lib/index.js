...
module.exports = function (moduleOptions) {
  ...
  this.nuxt.hook('tailwindcss:config', (config) => {
    ...
    config.theme.extend.zIndex = defu(config.theme.extend.zIndex, { '-1': '-1', 'dw-datepicker': 9000, 'dw-modal': 9001, 'dw-notification': 9002 })
    config.theme.extend.spacing = defu(config.theme.extend.spacing, { full: '100%' })
    config.theme.extend.minWidth = defu(config.theme.extend.minWidth, { 12: '3rem' })
