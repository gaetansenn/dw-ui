  ...
  const defaultConfig = {
    ...
    Step: {
      ...
      line: {
        ...
        classes (context) {
          return `w-1/${context.size} ${context.steps.value - 1 > context.index ? 'bg-gray-600' : 'bg-gray-400'}`
        }
      },
      title: {
    ...
    Toggle: {
      wrapper: 'relative rounded-full bg-blue-500 w-16 h-8 block cursor-pointer',
      input: 'hidden',
      toggle: {
        wrapper: 'absolute w-full h-full px-2 py-1',
        fixed: 'bg-white w-1/2 h-full rounded-full transform transition-transform ease-in duration-100 translate-x-0 border border-gray-400',
        classes (context) {
