  ...
  const defaultConfig = {
    ...
    Modal: {
      overlay: {
        ...
        fixed: 'fixed inset-0 z-50 overflow-y-auto'
      },
      backdrop: {
    ...
    Button: {
      fixed: 'block transition ease-in-out duration-150 flex items-center justify-center border',
      classes: (context) => {
        if (context.block) return 'w-full'
      },
      size: (props) => {
        return ({
