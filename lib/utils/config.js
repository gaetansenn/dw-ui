...
export default function () {
  ...
  ['InputText', 'InputEmail', 'InputPassword', 'InputRadio', 'InputDate', 'InputDatePicker', 'InputCheckbox'].forEach((type) => {
    if (!config.ui[type])
      ...
      config.ui[type].Input = defu({}, config.ui[type].Input, defaultConfig[type]?.Input || {}, config.ui.Input, Input)
  });
  ...
  ['Modal'].forEach((type) => {
    if (!config.ui[type])
      ...
      config.ui[type].Card = defu({}, config.ui[type].Card, defaultConfig[type]?.Card || {}, config.ui.Card, Card)
  });
  ...
  ['Dialog'].forEach((type) => {
    if (!config.ui[type])
      ...
      config.ui[type].Modal = defu({}, defaultConfig[type]?.Modal || {}, config.ui.Modal, Modal)
    else if (config.ui[type].Modal)
      config.ui[type].Modal = defu({}, config.ui[type].Modal, defaultConfig[type]?.Modal || {}, config.ui.Modal, Modal)
  })
