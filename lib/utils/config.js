...
export default function () {
  ...
  const defaultConfig = {
    ...
    InputGroup: {
      ...
      wrapper: {
        ...
        help: {
          fixed: 'text-left ml-1 mt-2 text-gray-500',
          size: (props) => {
            return ({
              xs: 'text-xs',
              sm: 'text-sm',
      ...
      label: {
        ...
        fixed: 'block font-light leading-5 text-black',
        classes: (props) => {
    ...
    Modal: {
      ...
      modal: {
        wrapper: 'transform transition-all overflow-hidden sm:w-full sm:max-w-lg text-center',
        transition: {
          enter: {
            fixed: 'ease-out duration-300',
        ...
        header: {
          fixed: 'font-semibold text-3xl leading-none text-left mr-8',
          icon: {
    ...
    InputDatePicker: {
      DatePicker: {
        fixed: 'fixed w-64 bg-white shadow-lg border border-gray-100 mt-2 p-2 z-dw-datepicker'
      },
      icon: {
    ...
    InputRadio: {
      fixed: 'inline-flex items-center',
      label: {
        fixed: 'pl-2 leading-none',
        star: 'text-red-500',
        size: (props) => {
          return ({
            xs: 'text-sm',
        ...
        validation: (props) => {
          if (!props.localeValidation) return ''

          return ({
            error: 'text-red-600'
          })[props.localeValidation.type]
        }
      },
      size: (props) => {
        return ({
    ...
    Button: {
      fixed: 'transition ease-in-out duration-150 items-center justify-center border disabled:cursor-not-allowed',
      classes: (context) => {
    ...
    Select: {
      wrapper: 'relative',
      fixed: 'border appearance-none focus:outline-none focus:shadow-outline w-full',
      variants: {
        default: ''
      },
    ...
    Step: {
      ...
      classes (context) {
        ...
        if (context.clickEvent) return 'cursor-pointer'
      },
      number: {
    ...
    TabsNav: {
      ...
      classes (context) {
        return `grid-cols-${context.tabs.length}`
      }
    },
    TabsNavItem: {
  ...
  ['InputText', 'InputEmail', 'InputPassword', 'InputRadio', 'InputDate', 'InputDatePicker', 'InputCheckbox'].forEach((type) => {
    if (!config.ui[type])
      config.ui[type] = {
