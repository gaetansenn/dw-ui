...
export default function () {
  ...
  const defaultConfig = {
    ...
    Dialog: {
      ...
      Modal: {
        Card: {
          content: {
            fixed: 'p-5 text-sm text-gray-500'
          },
          footer: {
  ...
  // Inject default Modal config
  ['Dialog'].forEach((type) => {
    if (!config.ui[type])
      config.ui[type] = {
        Modal: defu({}, defaultConfig[type]?.Modal || {}, config.ui.Modal, Modal)
      }
    else if (!config.ui[type].Modal)
      config.ui[type].Modal = defu({}, defaultConfig[type]?.Modal || {}, config.ui.Card, Modal)
    else if (config.ui[type].Modal)
      config.ui[type].Modal = defu({}, defaultConfig[type]?.Modal || {}, config.ui[type].Modal, config.ui.Input, Modal)
  })
