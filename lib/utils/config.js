...
export default function () {
  ...
  const Input = {
    ...
    validation: (props) => {
      const validation = props.validation || props.localValidation
    ...
    icon: {
      ...
      trailing: {
        fixed: 'absolute inset-y-0 right-0 flex items-center pr-4 text-gray-700',
        classes: ''
      },
      leading: {
        fixed: 'absolute inset-y-0 left-0 flex items-center pl-4 text-gray-700',
        classes: ''
      }
    },
    rounded,
  ...
  const Modal = {
    ...
    modal: {
      ...
      header: {
        fixed: 'font-semibold text-2xl sm:text-3xl leading-tight text-left mr-8',
        icon: {
  ...
  const defaultConfig = {
    ...
    Transitions,
    TextArea: {
      fixed: 'form-textarea transition duration-150 border ease-in-out disabled:cursor-not-allowed w-full placeholder-gray-500',
      remaining: 'text-sm text-gray-500 text-right pt-2'
    },
    InputGroup,
    InputGroupRadio: {
    ...
    DatePicker: {
      ...
      loading: {
        wrapper: 'flex flex-col space-y-2 items-center py-20',
        label: 'text-sm',
    ...
    Select: {
      ...
      icon: {
        ...
        icon: '<svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6 8l4 4 4-4"></path></svg>',
        size (context) {
          return ({
            xs: 'px-1',
            sm: 'px-2',
    ...
    i18n: {
      en: {
        ...
        InputPassword: {
          ...
          valid: 'Your password must be at least 8 characters long and include: uppercase, lowercase, number and special character'
        },
        InputNumber: {
          valid: 'Please provide a number'
        },
        InputDate: {
          valid: 'Please provide a valid date'
        },
        ...
        TextArea: {
          remaining: '{0} remaining characters'
        }
      },
      fr: {
        RichSelect: {
        ...
        InputPassword: {
          ...
          valid: 'Votre mot de passe doit être composé d\'au moins 8 caractères et inclure : une majuscule, une minuscule, un chiffre et un caractère spécial'
        },
        InputNumber: {
          valid: 'Veuillez indiquer un nombre'
        },
        InputDate: {
          valid: 'Veuillez indiquer une date valide'
        },
    ...
    Toggle: {
      wrapper: {
        fixed: 'relative rounded-full block cursor-pointer focus:outline-none focus:shadow-outline transition-colors ease-in duration-100',
        classes (context) {
  ...
  ['InputText', 'InputEmail', 'InputPassword', 'InputRadio', 'InputDate', 'InputDatePicker', 'InputCheckbox', 'TextArea', 'InputTime', 'InputDateTime', 'InputNumber'].forEach(parentPath => mergeClasses(Input, parentPath, 'Input', defaultConfig, newConfig))
  mergeClasses(Card, 'Modal', 'Card', defaultConfig, newConfig)
  mergeClasses(Modal, 'Dialog', 'Modal', defaultConfig, newConfig)
  mergeClasses(InputGroup, 'InputGroupCheckbox', 'InputGroup', defaultConfig, newConfig)
